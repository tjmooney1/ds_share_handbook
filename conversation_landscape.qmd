...
It's worth pointing out, that there are ways for the user to interactively explore the landscape at this stage, scanning over each node for its contents becoming familiar with each region of the landscape before clustering. The `plotly` package serves as a user friendly means for this purpose, helping us gather a 'lay of the land' and identify the dense and not so dense sections of our data.
...
We'll ideally keep some formatting consistencies when plotting our data, as we have mentioned previously, the colouring of our topic, ensuring they match up to their previous representation in any earlier visuals such as the landscape output. We also want to ensure that any visual we create ideally orders our topics by volume. With a few extra lines of code, we can ensure that these two things are possible.

First, we'll set the factor levels of our topics using the `dplyr::count()` output when called on the `topic_name` column, and the levels feature of the `factor()` base function like so:
```{r}
# sort topics by order of volume
topic_order <- data %>% dplyr::count(topic_name, sort = TRUE)
# set levels determined by volume of topic, this orders the group when plotting
data <- data %>% 
  dplyr::mutate(topic_name = factor(topic_name, levels = topic_order$topic_name))
```

##### Topic Volume over Time
Starting with volume over time, we often choose to render a faceted plot that includes all topics and their VOT for comparison. We can do so by using functionality found in packages such as `JPackage`.
```{r}
...
Next, we might want/need to break each of our topics out by their sentiment distribution to help shine light on any of particular interest or to help us tell a more refined story using our topic model. This can be done by using the `dr_plot_sent_group()` function of `DisplayR`.
```{r}
data %>% 
...
While the two visuals we have displayed so far are relatively basic and nearly always used, its not to say we won't require alternative methods to display topic-level information. Often, we render ngrams per topic to display the relationships that exist between terms/phrases and we may create plots to showcase things such as data source or Social Network/Platform distributions within topics.

With that said, it's worth noting that the need for specific methods entirely depends on the project domain and brief, as well as any outcome/findings derived, meaning we aught to be flexible in our approach to utilising data visualisation techniques to help support our project narrative.